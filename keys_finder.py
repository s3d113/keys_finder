...
os.system(SCAN)
KEY_GREP = "curl -is -X POST --url http://localhost:8000/api/v1/report_json --data 'hash=%s&scan_type=apk' -H 'Authorization:%s'  | grep -o -i -P '.{0,14}_key.{0,50}' | awk '{print $1, $3}' | tr -d '\\' 2> /dev/null | tr -d '\"' 2> /dev/null | tr -d ',' 2> /dev/null" % (UPLOAD_HASH, API_KEY)
os.system(KEY_GREP)
